...
local set_native_keymaps = function(_)
  ...
  keymap.register(e_mode.NORMAL, 'bc', TOOLS.wrap_f(HELPER.wipeout_buffer, nil, function(bufnr)
    return not this.api.is_pinned(bufnr)
  end), { remap = false, desc = 'wipeout current buffer.' })
  keymap.register(e_mode.NORMAL, 'bo', TOOLS.wrap_f(HELPER.wipeout_all_buffers, function(bufnr)
